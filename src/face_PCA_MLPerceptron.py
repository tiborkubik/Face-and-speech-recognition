...
def performPCA(toTrain_2d):
    ...
    pca = PCA(n_components = COMPONENTS,
	 		  svd_solver = 'full',
			  #whiten = True,
			  random_state = 10).fit(toTrain_2d)
    print("STEP3 done.")
...
def setUpClassifier():
    ...
    classifier = MLPClassifier( hidden_layer_sizes=(1600,),
                                solver = 'adam',
                                activation='logistic',
                                batch_size = 'auto',
                                alpha = 0.0001,
...
def main():
    ...
    for filename in filenames:
        ...
        CLASSES.extend([filename] * len(person))  # array of names of classes corresponding with each person
